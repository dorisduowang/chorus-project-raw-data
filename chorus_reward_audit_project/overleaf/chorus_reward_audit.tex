...
\title{\vspace{-1.3cm}When an AI Assistant Learns the Wrong Signal:\\
A Reproducible Audit of Attention Allocation in CHORUS}
\author{Doris Wang\\
...
\begin{abstract}
This project studies a small but consequential failure mode in AI assisted
knowledge work. CHORUS is a research assistant built for a scientific lab. It
uses structured registry data, semantic retrieval, and a directed hypergraph to
surface papers, people, projects, and computational resources. The system is
useful only if its ranking signal sends attention toward the right objects. I
audit what happens when a system treats textual surprise as a proxy for
intellectual value. The recovered project folder contained raw registry and
hypergraph data, but the original analysis code for the Memo 9 audit was
missing. I rebuilt the analysis from the raw data, created a reproducible
paper level panel, and regenerated three diagnostic figures. The audit shows
that a text focused reward can prefer papers that look complex on the surface
while ranking quieter papers with stronger empirical signals too low. The larger
point is not about one lab or one retrieval tool. It is about a general problem
in AI advice systems. An AI assistant does not only answer questions. It also
decides what becomes visible, what remains buried, and which users are best
positioned to act on the signal.
\end{abstract}

\section{Research Question}

The project asks whether an AI assistant can misallocate attention when it uses
a convenient textual proxy as the reward signal for ranking. The practical
version of the question is simple. If CHORUS is asked to surface interesting
scientific work, does it actually surface work with stronger evidence of
scientific importance, or does it surface work whose titles merely look
surprising?
...
This project treats CHORUS as a test case for that mechanism. The system is not
an entrepreneurship product, but the analytic structure is similar to AI advice
in entrepreneurial settings. A user faces many possible actions. The AI system
compresses the choice set. The user then selects from the compressed set and
decides what to do. If the compression is governed by a weak proxy, the AI can
increase confidence while reducing judgment quality.
...
surprise, but it may actually reward title verbosity. If so, the failure is not
just noisy ranking. It is a systematic inversion of attention. The system would
look active and discriminating while selecting the wrong part of the research
space.
...
older Memo 9 sample size. Memo 9 described 443 papers. The recovered hypergraph
contains 467 document nodes. The registry contains 482 profile publication rows
because some publications appear in more than one profile before document
consolidation.
...
the kind of surface textual feature that a weak reward signal can overvalue.
...
count bins. If title verbosity were a reasonable stand in for scientific value,
the long title bins should not be sharply worse on a simple impact measure. The
rebuild shows the opposite. Papers with six or fewer title words average 152
citations. Papers with twenty or more words average 21 citations. The ratio is
7.3 to 1 in favor of the shorter title bin.

This does not mean every short title is valuable or every long title is weak.
It also does not mean citations are a complete measure of intellectual value.
...
false positives. Hidden Gems average 103 citations, compared with 52 for False
...
The citation pattern supports the concern. Hidden Gems average 103 citations.
False Positives average 52 citations. The gap is about 2.0 to 1. The result is
smaller than the old Memo 9 figure because the recovered data snapshot is not
identical to the earlier 443 paper sample, but the direction is the same. The
text heavy signal is not merely adding noise. It is shifting attention toward a
different region of the research space.
...
shows that solo PhD linked papers average 117 citations, while solo faculty
...
is a search problem. The CHORUS audit shows a parallel search problem inside a
...
The CHORUS audit shows how an AI assistant can fail even when it is technically
useful. The failure is not that the system cannot retrieve papers. The failure
is that the reward signal can learn the wrong definition of what deserves
attention. In the rebuilt data, short titled papers have much higher average
citations than long titled papers, quiet high contrast papers outperform
verbose false positives, and author role composition changes how the signal
