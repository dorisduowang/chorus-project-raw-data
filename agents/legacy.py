"""
Legacy Chorus Agent - preserved from core_agent.py for backward compatibility.

This module contains the original single-agent ChorusAgent class and supporting
utilities (call_rag_async, call_rag, etc.) that were in the project-root
core_agent.py. They are kept here so that:

  * slack_bot.py and test files can import from the agents package instead
    of reaching into the deprecated core_agent module.
  * The ChorusOrchestrator (agents/orchestrator.py) remains the recommended
    entry point; ChorusAgent is the legacy fallback.

Typical usage:
    from agents.legacy import ChorusAgent, call_rag_async
    # or, via the package re-export:
    from agents import ChorusAgent, call_rag_async
"""
...
async def query(message: str) -> str:
    ...
    Usage:
        from agents.legacy import query
